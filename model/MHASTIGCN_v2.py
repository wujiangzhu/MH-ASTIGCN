...
class TMultiHeadAttention(nn.Module):
    def __init__(self, DEVICE, d_model, d_k ,d_v, n_heads, num_of_d):   #d_model=N
        ...
        self.num_of_d = num_of_d
        self.DEVICE = DEVICE
        self.W_Q = nn.Linear(d_model, d_k * n_heads, bias=False)    #直接映射为d_k*head,再分就是多头注意力的q了
        self.W_K = nn.Linear(d_model, d_k * n_heads, bias=False)
        self.W_V = nn.Linear(d_model, d_v * n_heads, bias=False)
        self.fc = nn.Linear(n_heads * d_v, d_model, bias=False)
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        residual, batch_size = input_Q, input_Q.size(0)
        # (B, S, D) -proj-> (B, S, D_new) -split-> (B, S, H, W) -trans-> (B, H, S, W)
        Q = self.W_Q(input_Q).view(batch_size, self.num_of_d, -1, self.n_heads, self.d_k).transpose(2, 3)  # Q: [batch_size,f=1, n_heads, len_q, d_k]
        K = self.W_K(input_K).view(batch_size, self.num_of_d, -1, self.n_heads, self.d_k).transpose(2, 3)  # K: [batch_size,f=1, n_heads, len_k, d_k]
        V = self.W_V(input_V).view(batch_size, self.num_of_d, -1, self.n_heads, self.d_v).transpose(2, 3)  # V: [batch_size,f=1, n_heads, len_v(=len_k), d_v]
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).repeat(1, self.n_heads, 1,
                                                  1)  # attn_mask : [batch_size, n_heads, seq_len, seq_len]
        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, attn_mask):
        ...
        if attn_mask is not None:   #mask肯定一个batch只有一个，形状[b,l,l],用于告诉哪些点间不需要注意力
            attn_mask = attn_mask.unsqueeze(1).repeat(1, self.n_heads, 1,   #因为是多头，所以要挤出一个维度，并复制成多头
                                                      1)  # attn_mask : [batch_size, n_heads, seq_len, seq_len]
        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
...
class cheb_conv_withSAt(nn.Module):
    ...
    def forward(self, x, spatial_attention, adj_pa):
        ...
        for time_step in range(num_of_timesteps):   #t个时刻就有t张图，进行t次图卷积

            graph_signal = x[:, :, :, time_step]  # (b, N, F_in)    #得到对应时刻的图信息

            output = torch.zeros(batch_size, num_of_vertices, self.out_channels).to(self.DEVICE)  # (b, N, F_out)

            for k in range(self.K): #开始k阶图卷积了

                T_k = self.cheb_polynomials[k]  # (N,N)
                mask = self.mask[k]

                myspatial_attention = spatial_attention[:, k, :, :] + adj_pa.mul(mask)
                myspatial_attention = F.softmax(myspatial_attention, dim=1) #注意力值激活一下，防止值过大过小
            ...
            outputs.append(output.unsqueeze(-1))  # (b, N, F_out, 1)    #因为要在最后一个维度拼接，所以另起一个维度

        return self.relu(torch.cat(outputs, dim=-1))  # (b, N, F_out, T)    #完成图卷积，输入到输出b，n,t都未变，只有f_int->f_out
...
class Embedding(nn.Module):
    ...
    def forward(self, x, batch_size):
        if self.Etype == 'T':   #时间嵌入时x-bftn     空间嵌入时，bfnt
            pos = torch.arange(self.nb_seq, dtype=torch.long).cuda()    #给序列编号0-nb_seq-1
            pos = pos.unsqueeze(0).unsqueeze(0).expand(batch_size, self.num_of_features,
                                                   self.nb_seq)  # [seq_len] -> [batch_size,F, seq_len=T]
...
class GDC(nn.Module):
    def __init__(self, in_channels, time_strides, kernel_size, dilation, padding):
        ...
        self.in_channels = in_channels  # =切比雪夫核的个数
        self.tanh = nn.Tanh()
    ...
    def forward(self, x):   # b=6 32 n=307 t=12
        x_causal_conv = self.con2out(x) # b=6 64 n=307 t=12
        x_causal_conv = self.bn(x_causal_conv)
        x_p = x_causal_conv[:, : self.in_channels, :, :]    #分成两半，前半部分
        x_q = x_causal_conv[:, -self.in_channels:, :, :]    #后半部分
        x_gtu = torch.mul(self.tanh(x_p), self.sigmoid(x_q))
        return x_gtu    # b f n t
...
class MHASTIGCN_block(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
        ...
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU(inplace=True)
        self.M = n_gdc  # gated dilated covolution的数量

        self.adj_pa = torch.FloatTensor(adj_pa).cuda()

        self.pre_conv = nn.Conv2d(num_of_timesteps, d_model, kernel_size=(1, num_of_d)) #num_of_d=F
        ...
        self.EmbedT2 = Embedding(num_of_timesteps, nb_chev_filter, num_of_vertices, 'T')    #第三个参数指要忽略的维度

        self.TAt = TMultiHeadAttention(DEVICE, num_of_vertices, d_k, d_v, n_heads, num_of_d)    #时间注意力，把t作为序列长度，点作为d
        self.SAt = SMultiHeadAttention(DEVICE, num_of_timesteps, d_k, d_v, n_heads, num_of_d)   #空间注意力，把N作为序列长度
        self.qt_att = MultiHeadAttention(DEVICE, 2 * d_model, d_k, d_v, K)  #只算出注意力，还未乘v

        self.cheb_conv_SAt = cheb_conv_withSAt(K, cheb_polynomials, in_channels, nb_chev_filter, num_of_vertices)

        self.T2At = T2MultiHeadAttention(DEVICE, nb_chev_filter, d_k, d_v, n_gdc, num_of_vertices)  #图的右上角的卷积
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.convs):
            fea = conv(gdc_input[:, :, i, :, :].squeeze(2).permute(0, 3, 1, 2)).unsqueeze_(dim=1)   # b 1 f n t
            if i == 0:
                gdc_output = fea
            else:
...
class MHASTIGCN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
        ...
        # len_input是12，输入数据的时间片长度
        self.final_conv = nn.Conv2d(int((len_input/time_strides) * nb_block), 128, kernel_size=(1, nb_time_filter)) #时间维度上卷积
        self.final_fc = nn.Linear(128, num_for_predict) #转换到指定维度
        self.DEVICE = DEVICE
    ...
    def forward(self, x):
        ...
        final_x = torch.cat(need_concat, dim=-1)    #在时间维度上拼接
...
def make_model(DEVICE, num_of_d, nb_block, in_channels, K,
    ...
    :return:
    '''
    L_tilde = scaled_Laplacian(adj_mx)  # tilde指上波浪线~，这里计算出归一化后的拉普拉斯矩阵
    cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(DEVICE) for i in cheb_polynomial(L_tilde, K)]    #计算切比雪夫多项式
