...
def get_adjacency_matrix2(distance_df_filename, num_of_vertices,
    ...
    Parameters
    ----------
    distance_df_filename: str, path of the csv file contains edges information  载入邻接矩阵的路径

    num_of_vertices: int, the number of vertices    点的数量

    type_: str, {connectivity, distance}    邻近矩阵
    ...
    A = np.zeros((int(num_of_vertices), int(num_of_vertices)),
                 dtype=np.float32)
    # 有的数据集各个探测器有编号，不一定是从0开始，所以存在记录编号文件的话要特殊考虑
    if id_filename:
        with open(id_filename, 'r') as f:
            id_dict = {int(i): idx
                       for idx, i in enumerate(f.read().strip().split('\n'))}   #将每一行内容和对应索引值存成列表，然后用字典映射编号到下标
        with open(distance_df_filename, 'r') as f:
            f.readline()    #跳过一行，第一行是标题
            reader = csv.reader(f)  #创建一个csv文件的读取器，会一行行的读取并自动解析成列表
            for row in reader:  #遍历读取器即遍历csv文件
                if len(row) != 3:
                    continue
                i, j, distance = int(row[0]), int(row[1]), float(row[2])
                A[id_dict[i], id_dict[j]] = 1   #将三元组的csv邻接矩阵转换为二维的邻接矩阵，换种表示
...
def scaled_Laplacian(W):
    ...
    assert W.shape[0] == W.shape[1] #检查是否是方阵

    D = np.diag(np.sum(W, axis=1))  #度矩阵，把每一行的和加起来放对角线上

    L = D - W

    lambda_max = eigs(L, k=1, which='LR')[0].real   #计算最大特征值，第一个参数是要计算的矩阵，第二个是最大的k个特征值和特征向量，第三个指最大实部
...
def cheb_polynomial(L_tilde, K):
    ...
    cheb_polynomials = [np.identity(N), L_tilde.copy()] #存拉普拉斯多项式，第0项是常数设为单位矩阵，第1项是本身
...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    ...
    file = os.path.basename(graph_signal_matrix_filename).split('.')[0] #从路径字符串提取文件名，并只取文件名，不要后缀

    dirpath = os.path.dirname(graph_signal_matrix_filename) #提取不带文件名的路径
    ...
    file_data = np.load(filename + '.npz')  #载入preparedata.py预处理后的数据
    train_x = file_data['train_x']  # (10181, 307, 3, 12)
    train_x = train_x[:, :, 0:1, :] #只要一个通道
    ...
    # ------- train_loader -------
    train_x_tensor = torch.from_numpy(train_x).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)  把numpy类型转为tensor
    train_target_tensor = torch.from_numpy(train_target).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)  训练的标签也要转换

    train_dataset = torch.utils.data.TensorDataset(train_x_tensor, train_target_tensor) #将输入数据和目标数据组合在一起

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)   #实例化一个数据迭代器
...
def compute_val_loss_mstgcn(net, val_loader, criterion, sw, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        val_loop = tqdm(enumerate(val_loader), total=len(val_loader))   #tqdm实例，展示进度条
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in pre_loop:
            ...
            encoder_inputs, labels = batch_data #b n f t   bnt
