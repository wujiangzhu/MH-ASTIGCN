...
def spatial_temporal_aware_distance(x, y):
    x, y = np.array(x), np.array(y) #【35,288】
    x_norm = (x**2).sum(axis=1, keepdims=True)**0.5 # 【35,1】
    y_norm = (y**2).sum(axis=1, keepdims=True)**0.5 #【35,1】
    p = x_norm[:, 0] / x_norm.sum() #【35】
    q = y_norm[:, 0] / y_norm.sum()  #【35】
    D = 1 - np.dot(x / x_norm, (y / y_norm).T)  #【35,35】
